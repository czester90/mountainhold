...
func test_ladder_cleanup_prunes_dead_unit_reservations() -> void:
	var e := _spawn()
	var ladder: Node3D = auto_free(SiegeLadder.instantiate())
	add_child(ladder)
	await await_millis(30)
	ladder.call("deploy", Vector3.ZERO, Vector3(3.0, 8.0, 0.0), Vector3.FORWARD)
	assert_bool(ladder.call("reserve_entry", e)).is_true()
	ladder.call("queue_point_for_unit", e)
	ladder.call("landing_settle_point_for_unit", e)
	assert_bool(ladder.call("reserve_climb", e)).is_true()
	e.set("_climbing_ladder", true)
	e.take_damage(1000.0)
	await await_millis(30)
	var summary: Dictionary = ladder.call("debug_summary")
	assert_int(summary["climbing"]).is_equal(0)
	assert_int(summary["entry"]).is_equal(0)
	assert_int(summary["queued"]).is_equal(0)
	assert_str(summary["state"]).is_equal("deployed")

func test_ladder_destroy_clears_reservations() -> void:
	var e := _spawn()
	var ladder: Node3D = auto_free(SiegeLadder.instantiate())
	add_child(ladder)
	await await_millis(30)
	...
	ladder.call("queue_point_for_unit", e)
	ladder.call("landing_settle_point_for_unit", e)
	ladder.call("take_damage", 1000.0)
	var summary: Dictionary = ladder.call("debug_summary")
	assert_int(summary["climbing"]).is_equal(0)
	assert_int(summary["entry"]).is_equal(0)
	assert_int(summary["queued"]).is_equal(0)
