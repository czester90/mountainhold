...
## a neutral-grey inspection terrain (full map, downsampled, height REDUCED), and
## proposes a "D-shaped" fortress: the mountain is the impassable rear (the flat
## side of the D); a SEMICIRCULAR wall at the mountain FOOT bulges out toward the
## enemy plain (the curved side of the D). Nothing is cropped/committed here.
##
## Controls:
...
const HEIGHT_SCALE := 900.0    # reduced: raw span 0.0739 -> ~66 m peak (smaller mountain)

# D-shaped fortress at the WEST foot of the mountain (world metres, 1024 m preview).
# Flat side of the D = mountain (x >= FOOT_X, rising east). Curved side = the
# semicircular wall bulging west into the flat enemy plain.
const FOOT_X := 338.0            # where wall meets the mountain base (N-S line)
const ARC_CZ := 500.0
const ARC_R := 40.0              # ~80 m fortress width across the diameter

const GATE_POS := Vector2(FOOT_X - ARC_R, ARC_CZ)   # apex of the arc, faces west

# Rectangular zones (crop straddles the terrain: west half = plain, east half = mountain).
const ZONES := [
	{"name": "PROPOSED CROP  384 x 384 m", "color": Color(1.0, 0.85, 0.1), "rect": [148, 308, 532, 692], "ly": 30.0},
	{"name": "PLAYABLE  160 x 180 m", "color": Color(0.2, 0.8, 1.0), "rect": [250, 410, 430, 590], "ly": 22.0},
	{"name": "ENEMY APPROACH (flat plain)", "color": Color(1.0, 0.3, 0.25), "rect": [238, 455, 298, 545], "ly": 12.0},
	{"name": "COURTYARD  ~30 x 35 m", "color": Color(0.35, 0.9, 0.4), "rect": [304, 478, 336, 522], "ly": 9.0},
]
...
func _ready() -> void:
	...
	_spawn = Vector3(250, _sample_h(250, 500) + 4.0, 500)
...
func _build_overlays() -> void:
	...
	# Semicircular wall (curved side of the D), bulging west toward the plain.
	_draped_arc(FOOT_X, ARC_CZ, ARC_R, PI / 2.0, 3.0 * PI / 2.0, Color(0.97, 0.97, 0.97), 6.0)
	# Back closure line against the mountain (flat side of the D).
	_draped_line(FOOT_X, ARC_CZ - ARC_R, FOOT_X, ARC_CZ + ARC_R, Color(0.6, 0.6, 0.72), 4.0)
	# Cliff connections where the arc ends meet the mountain base.
	_draped_line(FOOT_X, ARC_CZ + ARC_R, FOOT_X + 22, ARC_CZ + ARC_R + 14, Color(0.6, 0.6, 0.72), 8.0)
	_draped_line(FOOT_X, ARC_CZ - ARC_R, FOOT_X + 22, ARC_CZ - ARC_R - 14, Color(0.6, 0.6, 0.72), 8.0)

	_add_marker(GATE_POS.x, GATE_POS.y, Color(1, 1, 1), "GATE (faces approach)")
	_add_label("SEMICIRCULAR WALL  ~6 m", FOOT_X - ARC_R * 0.7, ARC_CZ + ARC_R * 0.7, 9.0, Color(0.97, 0.97, 0.97))
	_add_label("LEFT CLIFF", FOOT_X + 26, ARC_CZ + ARC_R + 18, 12.0, Color(0.72, 0.72, 0.82))
	_add_label("RIGHT CLIFF", FOOT_X + 26, ARC_CZ - ARC_R - 18, 12.0, Color(0.72, 0.72, 0.82))
	_add_label("MOUNTAIN  (rear, impassable)", 470, 545, 30.0, Color(0.85, 0.85, 0.9))
	_add_label("N", 380, 320, 20.0, Color(1, 1, 1))
...
func _draped_arc(cx: float, cz: float, r: float, a0: float, a1: float, col: Color, height: float) -> void:
	var n: int = maxi(16, int(r * absf(a1 - a0) / 4.0))
	var pts: PackedVector2Array = []
	for i in n + 1:
		var a: float = lerp(a0, a1, float(i) / n)
		pts.append(Vector2(cx + r * cos(a), cz + r * sin(a)))
	for i in n:
		_segment(pts[i], pts[i + 1], col, height, 1.2)

func _draped_line(x0: float, z0: float, x1: float, z1: float, col: Color, height: float) -> void:
	_segment(Vector2(x0, z0), Vector2(x1, z1), col, height, 1.2)

func _segment(a: Vector2, b: Vector2, col: Color, height: float, thick: float) -> void:
	...
	var box := BoxMesh.new()
	box.size = Vector3(length / n + 0.5, height, thick)
	mm.mesh = box
	mm.instance_count = n
	...
	for i in n:
		...
		mm.set_instance_transform(i, Transform3D(Basis(Vector3.UP, yaw), Vector3(p.x, h + height * 0.35, p.y)))
...
func _toggle_top_down() -> void:
	...
	if _top_down:
		_cam.global_position = Vector3(340, 300, 500)
...
func _process(delta: float) -> void:
	...
	if dir != Vector3.ZERO:
		var speed := 90.0 if Input.is_key_pressed(KEY_SHIFT) else 28.0
